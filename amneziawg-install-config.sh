...
checkPackageAndInstall() {
    local name="$1"
    local isRequried="$2"
    # проверяем, установлена ли библиотека $name
    if opkg list-installed | grep -q $name; then
...
check_request() {
    ...
    response_body="${response%???}"  # Всё, кроме последних 3 символов - это тело ответа
    # echo $response_body
    # echo $response_code
    ...
    if opkg list-installed | grep -q $PACK_NAME; then
        ...
        for pkg in $PACKAGES; do
            ...
            if opkg list-installed | grep -q "^$pkg "; then
                ...
                echo "$pkg not installed. Install..."
                opkg install $pkg
                if [ $? -eq 0 ]; then
...
while [ $currIter -lt $countRepeatAWGGen ] && [ "$isExit" = "0" ]; do
    ...
    if [ "$warp_config" = "Error" ]; then
        ...
        # вытаскиваем нужные нам данные из распарсенного ответа
